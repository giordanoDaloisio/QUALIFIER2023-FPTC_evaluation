...
def compute_fptc(path, df_names, label, slope=''):
    ...
    for data in df_names:
        ...
        iterations.append(len(classifier.estimators_))
        n = adult_x.shape[0]
        v = adult_x.shape[1]
        ...
        attributes.append(n)
        if slope == '':
          f_classifier = len(classifier.estimators_) * (m + 1) * n * v * log2(n)
        else:
          f_classifier = len(classifier.estimators_) * (m + 1) * n * v * log2(n) * slope
    ...
    return pd.DataFrame({'Training time': training_times, 'FPTC': fs_classifier, 'Trees': iterations})
